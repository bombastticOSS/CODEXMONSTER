...
st.session\_state.escala = nova.reset\_index()
st.session\_state.ultima\_otimizacao = "Concluída agora"
st.session\_state.revisao\_grade = st.session\_state.get("revisao\_grade", 0) + 1
salvar\_estado()
qualidade = "ótima" if status == cp\_model.OPTIMAL else "viável"
return True, (
	f"Nova alternativa {qualidade} gerada. Todas as "
	f"{sum(map(len, st.session\_state.travas.values()))} travas foram preservadas."
)
...
def exibir_grade_mensal() -> None:
    ...
    st.caption(
        "Clique em uma célula livre para escolher o código. Uma edição manual vira uma trava automaticamente; "
        "o símbolo 🔒 indica que a otimização não poderá alterá-la. FP é uma preferência, não uma trava."
    )
    st.markdown(
        "<div class='legenda'>"
        "<span class='m6'>M6 · manhã</span><span class='d12'>D12 · diurno</span><span class='n12'>N12 · noturno</span>"
        "<span class='fp'>FP · folga pedida</span><span class='fe'>FE · férias</span><span class='at'>AT · atestado</span>"
        "<span class='lm'>LM · licença-maternidade</span><span class='lic'>LIC · licença pessoal</span>"
        "<span class='lock'>🔒 célula protegida</span><span class='error'>vermelho · conflito/inviabilidade</span>"
        "</div>",
